...
void prvReadFromUart(void *pvparameters)
{
	...
	while(1) {
		if(xUartRecvQueue != NULL) {
			_len = SYS_CONSOLE_Read(SYS_CONSOLE_INDEX_1, STDIN_FILENO, rdUartTmpBuf, 1);
			while(xSemaphoreTake(xUartRdSemaphore, cDelaySec) != pdPASS) {}
			if(rdUartSize > 0) {
				i = 0;
				while(i < rdUartSize) {
	...
	case 't':
		...
		if(num < LMT01_SENSOR_NUM) {
			if(DRV_TEMP_LM01_StartConversion(&(x_Temp[num]))) {
				...
				tval = DRV_TEMP_EndConversion(&(x_Temp[num]));
	...
	while(1) {
		...
		for(i = 0; i < LMT01_SENSOR_NUM; i++) {
			if(DRV_TEMP_LM01_StartConversion(&(x_Temp[i]))) {
				vTaskDelay(cTick110ms);
				tval = DRV_TEMP_EndConversion(&(x_Temp[i]));
